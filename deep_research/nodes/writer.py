"""Writer node — synthesizes the final Markdown report (deep path).

P3: implemented via `prompts/writer.txt`.
"""
...
async def write(state: ResearchState, client: AsyncOpenAI, model: str) -> str:
    """Call the writer LLM with the planner + drafts + citations and return Markdown."""
    ...
    try:
        ...
        # Defensive cleanup: if the LLM wrapped the markdown in fences, strip them
        if md.startswith("```"):
            lines = md.splitlines()
            if len(lines) >= 2:
        ...
        # Fallback: concatenate drafts as the report
...
def _concatenate_drafts(state: ResearchState) -> str:
    """Ghetto fallback if the writer fails."""
    parts = [f"# Report\n\nQuery: {state.query}\n"]
    for sq in state.plan.sub_questions:
        draft = state.drafts.get(sq.id)
        if draft:
