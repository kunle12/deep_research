...
- `merge <run_id1> <run_id2> [more...] [--name "Combined"] [--delete-sources]` —
  ...
  merging tags); a deterministic stitch is used if the LLM call fails. When no
  `--name` is given, the merged report's **title is regenerated by the LLM**
  (a fresh, descriptive headline) rather than concatenating the source queries.
  Source reports are kept and tagged `merged` unless `--delete-sources` is
  passed, in which case their analyses are reassigned to the merged report
  before they are deleted. The web UI has a "Merge with…" panel on the report
  page.
...
The report page shows the report's **proper title** (derived from its first
markdown heading) rather than the raw research query — the original query is
kept behind a small superscript query icon: click it to reveal the query text,
click anywhere else to dismiss. **Rename** edits the title and keeps the
markdown heading in sync; **Add source** (paste a URL to fully analyze and
attach it to this research — runs as a tracked job); and a **Merge with…**
panel to combine it with other reports into one unified research (optional new
name, optional delete sources).
