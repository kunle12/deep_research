...
async def test_upsert_overwrites(sqlite_backend):
    now = datetime.now(UTC).isoformat()
    e1 = GlossaryEntry(
        term="AI", term_canonical="ai", kind="concept",
        short_def="Original def", last_updated=now,
    )
    await sqlite_backend.upsert_glossary_entry(e1)

    e2 = GlossaryEntry(
        term="AI", term_canonical="ai", kind="concept",
        short_def="Updated def", last_updated=now,
    )
...
async def test_list_glossary(sqlite_backend):
    ...
    for term, canonical in [("A", "a"), ("B", "b"), ("C", "c")]:
        await sqlite_backend.upsert_glossary_entry(GlossaryEntry(
            term=term, term_canonical=canonical, kind="concept",
            short_def=f"Definition {term}", last_updated=now,
        ))
