...
@pytest.mark.asyncio
async def test_merge_title_regenerated_when_unspecified(sqlite_backend, tmp_path):
    from deep_research.library.merge import merge_reports
    from deep_research.library.writer import LibraryWriter
    ...
    # With no explicit name, the title is regenerated by the LLM (not a simple
    # concatenation of the source queries).
    new_id = await merge_reports(
        sqlite_backend, writer, ["run_a", "run_b"], _fake_router(_fake_llm("# Unified Topic"))
    )
    report = await sqlite_backend.get_report(new_id)
    assert report is not None
    assert report.original_query == "Unified Topic"
    # The merged markdown carries the regenerated title as an H1 heading.
    assert re.search(r"^# Unified Topic$", report.markdown, re.M) is not None
